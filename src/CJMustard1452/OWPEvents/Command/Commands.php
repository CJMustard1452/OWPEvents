...
namespace CJMustard1452\OWPEvents\Command;
	...
	public function commandEvent(CommandEvent $event){
		...
		if(explode(" ", $event->getCommand())[0] === "owpevent"){
			if($sender->hasPermission("owpe-eventscommand.manage")){
				...
				$form->setTitle("§l§7(§bOwnage Events§7)");
				$form->addLabel("As an event manager you may create/join events within this menu");
				$form->addDropdown("Choose Option", ["Join Event", "Host Event"]);
				$form->sendToPlayer($sender);
			}
		}
	}
